...
class HTMLJobExtractor:
    ...
    def _extract_with_selector(self, soup: BeautifulSoup, config: Dict[str, Any], source_url: str = "") -> Optional[str]:
        """
        ...
        if transform in transformations:
            ...
            if transform_type == 'regex_replace':
                ...
                for line in lines:
                    ...
                    elif line and len(line.strip()) > 10:  # Assume non-bullet lines that are long enough are items
    ...
    def _extract_skills(self, text_content: str) -> Dict[str, List[str]]:
        ...
        required_skills = []
        preferred_skills = []  # Apple jobs don't typically have separate preferred skills sections
    ...
    def _determine_experience_level(self, title: str, description: str) -> Optional[ExperienceLevel]:
    ...
    def extract_job_posting(self, html_content: str, source_url: str = "", filename: str = "") -> Optional[JobPosting]:
        """
        ...
        try:
            ...
            if not job_id or not title:
                self.logger.warning(f"Missing required fields in {filename}: job_id={job_id}, title={title}")
                return None
            
            # Extract time information
            time_config = self.patterns.get('time_info', {})
            publish_date = self._extract_with_selector(soup, time_config.get('publish_date'), source_url)
            scraped_date = self._extract_with_selector(soup, time_config.get('scraped_date'), source_url)
            
            # Extract location
            location_config = self.patterns.get('location', {})
            location_text = self._extract_with_selector(soup, location_config.get('location'), source_url)
            ...
            description = self._extract_with_selector(soup, content_config.get('description'), source_url)
            pay_benefit = self._extract_with_selector(soup, content_config.get('pay_benefit'), source_url)
            
            # Extract qualifications as lists
            minimum_qualifications = self._extract_list_items(soup, content_config.get('required_qualifications', {}).get('selector', ''))
            preferred_qualifications = self._extract_list_items(soup, content_config.get('preferred_qualifications', {}).get('selector', ''))
            
            # Also get text versions for skills extraction
            required_quals_text = self._extract_with_selector(soup, content_config.get('required_qualifications'), source_url)
            preferred_quals_text = self._extract_with_selector(soup, content_config.get('preferred_qualifications'), source_url)
            
            # Extract team information
            team_config = self.patterns.get('team_info', {})
            team = self._extract_with_selector(soup, team_config.get('team'), source_url)
            department = self._extract_with_selector(soup, team_config.get('department'), source_url)
            
            # Extract application information
            app_config = self.patterns.get('application', {})
            application_url = self._extract_with_selector(soup, app_config.get('application_url'), source_url)
            ...
            experience_level = self._determine_experience_level(title or "", f"{description or ''} {combined_quals}")
            job_type = self._determine_job_type(title or "", description or "")
            work_arrangement = self._determine_work_arrangement(f"{description or ''} {combined_quals}")
            
            # Extract additional fields from basic_info
            source_platform = self._extract_with_selector(soup, basic_config.get('source_platform'), source_url)
            if not source_url and basic_config.get('source_url'):
                source_url = self._extract_with_selector(soup, basic_config.get('source_url'), source_url) or ""
            ...
            job_posting = JobPosting(
                ...
                experience_requirements=self._extract_experience_requirements(minimum_qualifications, preferred_qualifications),
    ...
    def _extract_education_requirements(self, qualifications: List[str]) -> List[str]:
        ...
        education_keywords = [
            'degree', 'bachelor', 'master', 'phd', 'bs', 'ba', 'ms', 'ma', 'mba',
            'education', 'university', 'college', 'graduate', 'undergraduate',
            'computer science', 'engineering', 'equivalent education'
        ]
    ...
    def _extract_experience_requirements(self, minimum_quals: List[str], preferred_quals: List[str]) -> List[str]:
        """Extract experience-specific requirements from qualifications lists."""
        experience_requirements = []
        
        # Experience-related keywords to look for
        experience_keywords = [
            'experience', 'years', 'background', 'familiar', 'knowledge of', 'understanding of',
            'skilled', 'proficient', 'expertise', 'working with', 'development', 'programming'
        ]
        ...
        for qual in minimum_quals:
            ...
            if any(edu_keyword in qual_lower for edu_keyword in ['degree', 'bachelor', 'master', 'phd', 'bs', 'ba', 'ms', 'ma', 'education', 'university', 'college']):
                continue
        ...
        for html_file in html_files:
            try:
                ...
                if job_posting:
                    ...
                    print(f"  ✗ Failed to extract job posting")
