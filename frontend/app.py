...
# Page configuration
st.set_page_config(
    page_title="Semantix", page_icon="🔍", layout="centered", initial_sidebar_state="collapsed"
)
...
def display_chat_history():
    ...
    if st.session_state.chat_history:
        ...
        for _i, chat in enumerate(st.session_state.chat_history):
            with st.container():
                category_display = (
                    ", ".join(chat["category"])
                    if isinstance(chat["category"], list)
                    else chat["category"]
                )
                st.markdown(
                    f"""
                ...
                if query != st.session_state.get("last_processed_query", ""):
                    ...
                    with st.spinner("Processing your request..."):
                        ...
                        st.session_state.chat_history.append(
                            {"query": query, "category": categories, "response": response}
                        )
                    # Clear the input
